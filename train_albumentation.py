...
from efficientdet.dataset import CocoDataset, CocoAlbumentationsDataset, Resizer, Normalizer, Augmenter, collater
...
def train(opt):
    ...
    if opt.optim == 'adamw':
        ...
        optimizer = torch.optim.SGD(model.parameters(), opt.lr, momentum=0.9, weight_decay=opt.weight_decay, nesterov=True)
    ...
    #####################################################################################
    # Check the validation loss for the first time
    # to determine the best_loss more accurately
    if opt.load_weights is not None:
        if opt.load_weights.endswith('.pth'):
            None
        else:
